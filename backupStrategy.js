...
for(let i = 5; i < arr.length; i++ ) {
    ...
    if(buy) {
        if((wma * .998 ) < arr[i].total) {
            ...
            if(shortPrice !== 0 ) {
                profitArr.push({index: i, side: 'Short', asset: assetName, profit: currProfit, date: spy[i].date,gains: profit})
            }
        ...
        if((wma * .998 ) > arr[i].total) {
            ...
            profit += currProfit
            profitArr.push({ index: i, side: 'Long', asset: assetName, profit: currProfit, date: spy[i].date, gains: profit})
